...
for index in range(3, groupsNumber + 1):
   ...
   def generateThirdTask():
    ...
    if (var % 3 == 0):
        ...
        b1 = randomNumbers(low, high, size)
        b2 = randomNumbers(-10, 10, 1) * a1 + randomNumbers(-10, 10, 1) * a2
        b3 = a1 + randomNumbers(-10, 10, 1) * a2
        b4 = b1 - 2 * b2
        mat_b = np.concatenate((b1, b2, b3, b4), axis=0)
        rank_l2 = la.matrix_rank(mat_b)
        mat_c = np.concatenate((a1, a2, b1, b2, b3), axis=0)
        ...
        b1 = randomNumbers(-10, 10, 1) * a2 + randomNumbers(-10, 10, 1) * a1
        b2 = randomNumbers(-10, 10, 1) * a1 + a2
        b3 = 2 * b1 - b2
        b4 = b1 - 2 * b2
        mat_b = np.concatenate((b1, b2, b3, b4), axis=0)
        rank_l2 = la.matrix_rank(mat_b)
...
def writeThirdTask(tasksFile, answersFile):
    ...
    tasksFile.write("\\$a_1$ = (" + str(mat_a[0,0]) + ", " + str(mat_a[0,1]) + ", " + str(mat_a[0,2]) + ", " + str(mat_a[0,3]) + "\\")
    tasksFile.write("\\$a_2$ = (" + str(mat_a[1,0]) + ", " + str(mat_a[1,1]) + ", " + str(mat_a[1,2]) + ", " + str(mat_a[1,3]) + "\\")
    tasksFile.write("\\$a_3$ = (" + str(mat_a[2,0]) + ", " + str(mat_a[2,1]) + ", " + str(mat_a[2,2]) + ", " + str(mat_a[2,3]) + "\\")
    tasksFile.write("\\$a_4$ = (" + str(mat_a[3,0]) + ", " + str(mat_a[3,1]) + ", " + str(mat_a[3,2]) + ", " + str(mat_a[3,3]) + "\\")
    tasksFile.write("a $L_2$~--- линейная оболочка векторов\n")
    tasksFile.write("\\$b_1$ = (" + str(mat_b[0,0]) + ", " + str(mat_b[0,1]) + ", " + str(mat_b[0,2]) + ", " + str(mat_b[0,3]) + "\\")
    tasksFile.write("\\$b_2$ = (" + str(mat_b[1,0]) + ", " + str(mat_b[1,1]) + ", " + str(mat_b[1,2]) + ", " + str(mat_b[1,3]) + "\\")
    tasksFile.write("\\$b_3$ = (" + str(mat_b[2,0]) + ", " + str(mat_b[2,1]) + ", " + str(mat_b[2,2]) + ", " + str(mat_b[2,3]) + "\\")
    tasksFile.write("\\$b_4$ = (" + str(mat_b[3,0]) + ", " + str(mat_b[3,1]) + ", " + str(mat_b[3,2]) + ", " + str(mat_b[3,3]) + "\\")
    tasksFile.write("\n")
